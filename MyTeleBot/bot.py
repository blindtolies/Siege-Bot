...
class SiegeBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Ensure user is remembered with admin status checked
        chat_id = update.message.chat_id
        if update.message.chat.type != 'private':
            await self.update_admins(chat_id, context)
        is_admin = self.is_admin(chat_id, update.effective_user.id)
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.message.chat.type != 'private':
            await self.update_admins(chat_id, context)
        is_admin = self.is_admin(chat_id, update.effective_user.id)
        self._remember_user(update, self._get_user_name(update), chat_id, is_admin)
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # 1. Update admin list and check status
        if update.message.chat.type != 'private':
            # This is an awaited call, which might cause a minor delay but ensures admin status is current
            await self.update_admins(chat_id, context)
        is_admin = self.is_admin(chat_id, user_id)
        
        # 2. Remember/update user data and admin status
        self._remember_user(update, user_name, chat_id, is_admin)
        
        # 3. Add message to history
        self._learn_from_conversation(user_id, update.message.text)
        
        # 4. Check for direct/instant sassy replies (bypasses LLM)
        ...
        # 5. Determine if the bot should use the LLM
        is_private = update.message.chat.type == 'private'
        is_mention = self.bot_username.lower() in update.message.text.lower()
        ...
        try:
            ...
            response = self.cohere_client.generate(
                model='command',
                prompt=prompt,
                max_tokens=self.config.max_response_length, # Use configured length
                temperature=0.7,
                stop_sequences=["\n\n", "Human:", "User:"]
            )
    ...
    def _learn_from_conversation(self, user_id, message):
        ...
        if user_id not in self.user_data:
            # Should not happen if _remember_user is called first, but as a safeguard
            return 
        ...
        # This overwrites the list with the cleaned version
        self.user_data[user_id]["history"] = [
            entry for entry in history 
            if entry["timestamp"] >= threshold_time
        ]

        # 2. Message count safety cleanup (if history is still too long)
    ...
    def _get_user_name(self, update: Update):
        # Use first_name if username is not set
    ...
    async def update_admins(self, chat_id, context):
        """Fetches and updates the admin list for a given chat."""
        try:
            admins = await context.bot.get_chat_administrators(chat_id)
            self.admins_per_chat[chat_id] = set(admin.user.id for admin in admins)
        except Exception as e:
            # This is normal if the bot is not an admin itself or the chat type is wrong
            logger.warning(f"Could not fetch admin list for chat {chat_id}: {e}")
    ...
    def _remember_user(self, update, user_name, chat_id, is_admin=False):
        ...
        # Always update admin status and username
